...
mod tests {
  ...
  fn donate() {
      ...
      contract.donate();
      let first_donation = contract.get_devisor_for_account("donor_a".parse().unwrap());
      ...
      contract.donate();
      let second_donation = contract.get_devisor_for_account("donor_b".parse().unwrap());
      ...
      assert_eq!(contract.number_of_devisors(), 2);
  }
